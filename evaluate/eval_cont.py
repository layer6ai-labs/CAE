...
def conv_eval(env_name):
    ...
    # call the policy
    action_policy = goal_conditioned_c_learning_policy(critic1, actor, exploration_rng, device=device,
                                                       action_dim=d_actions,
                                                       eval=True, g_norm=g_norm, o_norm=o_norm)

    success = []
    step_goal = []

    #evaluate for 500 goals
    for i in range(500):
        su= 0
    ...
    print("success rate:")
    print("mean:", np.mean(success))
    print("std:", np.std(success))
    step_goal = np.array(step_goal)
    print("step to goal:")
    print("mean:", np.mean(step_goal))
    print("std:", np.std(step_goal))
